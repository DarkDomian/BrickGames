 ...
 * @post ui->game_field, ui->info_bar, and ui->next_field are valid window pointers
 ...
 * Each cell is rendered as either FILLED_PIXEL or EMPTY_PIXEL based on its value.
 *
 ...
 */
void PrintMatrix(WINDOW *screen, int **matrix, int row, int col, 
                 int shift_row, int shift_col);

/**
 * @brief Main game event loop
 *
 * Handles input processing, game state updates, and UI refresh in a continuous loop.
 * The loop runs until a termination action is received.
 *
 * @param ui Pointer to BrickUI structure with initialized windows
