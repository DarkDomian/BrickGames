 ...
 * The API follows BrickGame specification, it's designed to be forward-compatible
 * for future game implementations across different interface types (console, desktop, etc.).
 */
 ...
 * 
 * This function is called by the interface when a button is pressed or released.
 * It translates physical button actions into game logic responses.
 * 
 * @param action The type of button action (from UserAction_t enum)
 * @param hold Indicates whether the button is being held down (true) or just pressed (false)
 * 
 * @note The effect of each action is game-dependent. For example:
 *       - In Tetris: Left/Right moves piece, Up rotates, Down speeds fall
 *       - In Snake: Left/Right/Up/Down changes direction
 *       - Start/Pause/Terminate have consistent meanings across games
 */
